  ...
  return (
    <div className='max-w-[1520] mx-auto flex justify-between items-center p-4'>
        <div className='flex items-center'>
            ...
            <h1 className='text-2xl sm:text-3xl lg:text-4xl px-2'>Living
        ...
        <button className='bg-orange-700 text-white hidden md:flex items-center p-2 rounded-full'>Cart
        <BsFillCartFill size={25}/>
        </button>

        {sideNav? (<div onClick={()=> setSideNav(!setSideNav)} className='bg-black/60 fixed w-full h-screen z-10 top-0 left-0'></div>): ("")}

        <div className={sideNav? 'fixed top-0 left-0 w-[300px] h-screen bg-white z-10 duration-300': 'fixed top-0 left-[-100%] h-screen w-[300px] bg-white z-10 duration-300'}>
            <AiOutlineClose size={25} onClick={()=> setSideNav(!setSideNav)}
            className='absolute cursor-pointer right-4 top-4'/>

            <h2 className='text-2xl p-4'>Living <span className='text-orange-700 font-bold'>Food</span> </h2>
